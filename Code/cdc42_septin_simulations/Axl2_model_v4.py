...
def formulate_components_fem_2d(mod_param, trial_f, trial_f_prev, test_f, measures, mesh, exp_list):
    ...
    Cdc42T, Cdc42D, BemGEF42, BemGEFm, S, P, GapS, Axl2, Axl2_S, FOO = trial_f
    Cdc42T_prev, Cdc42D_prev, BemGEF42_prev, BemGEFm_prev, S_prev, P_prev, GapS_prev, Axl2_prev, Axl2_S_prev, FOO_prev = trial_f_prev
    phi1, phi2, phi3, phi4, phi5, phi6, phi7, phi8, phi9, phi10 = test_f
    Cdc42c_exp, BemGEFc_exp, Sc_exp, GapSc_exp, Axl2c_exp = exp_list

    dx = measures
    
    mass_form, stiff_form, reaction_form = 0, 0, 0
    k1a, k1b, k2a, k2b, k3, k4a, k4b, k5a, k5b, k7, k12a, k12b, k13, k14, k15, k16, k17, k18, k19, k20, k21, k22, k24, k25, Dm, Dms, Dmss_val, Dc, eta, Dm_gdp = mod_param.make_param_constants()
    ...
    FOO_mass = (FOO - FOO_prev) * phi10 * dx    
    ...
    if mod_param.GapSlim != False:
        k12a_val = Constant(mod_param.k12a)
        k12a = conditional(lt(GapS_prev, mod_param.GapSlim), k12a_val, 0.0)

    DmFOO = Constant(mod_param.DmFOO)
    ...
    GapS_stiff = Dmss * dot(grad(GapS), grad(phi7)) * dx
    Axl2_stiff = Dms * dot(grad(Axl2), grad(phi8)) * dx
    Axl2_S_stiff = Dms * dot(grad(Axl2_S), grad(phi9)) * dx
    FOO_stiff = DmFOO * dot(grad(FOO), grad(phi10)) * dx
    ...
    # S 
    f5 = k15*Axl2_S - (2*k16*S_prev*S_prev + k17_term) + k18_use*P - k21*S - k19*Axl2_prev*S_prev + k25*FOO_prev*Sc_exp
    ...
    reaction_form += (-f1*phi1 - f2*phi2 - f3*phi3 - f4*phi4 - f5*phi5 - f6*phi6 - f7*phi7 - f8*phi8 - f9*phi9 - f10*phi10) * dx
    mass_form += Cdc42T_mass + Cdc42D_mass + BemGEF42_mass + BemGEFm_mass + S_mass + P_mass + GapS_mass + Axl2_mass + Axl2_S_mass + FOO_mass
    stiff_form += Cdc42T_stiff + Cdc42D_stiff + BemGEF42_stiff + BemGEFm_stiff + S_stiff + P_stiff + GapS_stiff + Axl2_stiff + Axl2_S_stiff + FOO_stiff
...
def solve_ode_system(U, mod_param, exp_list, dt, eta_div_cell_area, dx):
    ...
    Cdc42T, Cdc42D, BemGEF42, BemGEFm, S, P, GapS, Axl2, Axl2_S, FOO = U.split()
    Cdc42c_exp, BemGEFc_exp, Sc_exp, GapSc_exp, Axl2c_exp = exp_list
    x_old = np.zeros(5)
    ...
    k1a, k1b, k2a, k2b, k3, k4a, k4b, k5a, k5b, k7, k12a, k12b, k13, k14, k15, k16, k17, k18, k19, k20, k21, k22, k24, k25, Dm, Dms, Dmss, Dc, eta, Dm_gdp = mod_param.return_param()
    ...
    ode_form3 = (eta_div_cell_area * (k21*S - k20*Axl2*Sc_exp - k25*FOO*Sc_exp))*dx
...
def compute_mass(U, exp_list, eta_div_cell_area, dx):
    Cdc42T, Cdc42D, BemGEF42, BemGEFm, S, P, GapS, Axl2, Axl2_S, FOO = U.split(True)
    Cdc42c_exp, BemGEFc_exp, Sc_exp, GapSc_exp, Axl2c_exp = exp_list
...
def calc_max_coord_cdc42(U_high, H):
    ...
    Cdc42T_tmp, tmp1, tmp2, tmp3, tmp4, P_tmp, tmp5, tmp6, tmp7, tmp8 = U_high.split(True)
    n_states = 10
...
def compute_save_dist_data(U_high, mesh, t, file_loc):
    ...
    Cdc42T, Cdc42D, BemGEF42, BemGEFm, S, P, GapS, Axl2, Axl2_S, FOO = U_high.split(True)
...
def solve_2d(file_loc, mod_param, sol_opt, bud_scar_param, exocyte_data):
    ...
    t_curr = 0.0
    _Cdc42T_high, _Cdc42D_high, _BemGEF42_high, _BemGEFm_high, _S_high, _P_high, _GapS_high, _Axl2_high, _Axl2_S_high, _FOO_high = U_high.split()
    Cdc42T_high, Cdc42D_high, BemGEF42_high, BemGEFm_high, S_high, P_high, GapS_high, Axl2_high, Axl2_S_high, FOO_high = split(U_high)
    ...
    vtkfile_FOO = File(file_loc.dir_pvd + "FOO" + ".pvd")
    ...
    # In case we only want to print eps-hit statistics
    if exocyte_data.only_print_eps_hit == True:
        Cdc42T_tmp, tmp1, tmp2, tmp3, tmp4, P_tmp, tmp5, tmp6, tmp7, tmp8 = U_high.split(True)
        i_hit = exocyte_data.calc_hit_window(Cdc42T_tmp)
        return None
    ...
    for i in tqdm(range(sol_opt.term_max_it)):
        ...
        if t_curr > t_hit:
            ...
            Cdc42T_tmp, tmp1, tmp2, tmp3, tmp4, P_tmp, tmp5, tmp6, tmp7, tmp8 = U_high.split(True)
            if exocyte_data.use_cables == False and exocyte_data.cables_windows == False:
                i_hit = exocyte_data.calc_site_hit(Cdc42T_tmp)
            elif exocyte_data.use_cables == False and exocyte_data.cables_windows == True:
                i_hit = exocyte_data.calc_hit_window(Cdc42T_tmp)
            else:
        ...
        if sol_opt.save_dist != False and (t_it % sol_opt.save_dist == 0 or t_it == 1):
...
def run_axl2_v4(file_loc, mod_param_list, sol_opt_list, times_run, n_threads, bud_scar_param=None, exocyte_data=None, couple_mod_param_and_exo=False):
