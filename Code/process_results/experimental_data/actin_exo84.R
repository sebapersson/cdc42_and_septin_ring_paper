...
mybins <- cut(data_f$volume, breaks=7)
data_foo <- data_f |>
...
mybins <- cut(data_e$volume, breaks=7)
data_foo <- data_e |>
  mutate(mybins = mybins) |>
  group_by(strain_type, mybins) |>
...
mybins <- cut(data_g$max_cdc10_diameter, breaks=7)
data_foo <- data_g |>
  mutate(mybins = mybins) |>
  group_by(strain_type, mybins) |>
...
# -----------------------------------------------------------------------------------------------------------------
# Phalloidin experimental data
# -----------------------------------------------------------------------------------------------------------------
my_col <- cbPalette[c(4, 2)]
SCALE_AREA <- 0.0865202^2
path_file <- file.path("..", "..", "..", "Data", "UTF-8v1_phalloidin_snapshots_actin_actin_cluster_size.csv")
data <- read_csv(path_file, col_types = cols()) |>
  mutate(volume = cell_vol_fl,
         cluster_area = actin_cluster_size * SCALE_AREA)
mybins <- cut(data$volume, breaks=3)
data_foo <- data |>
  mutate(mybins = mybins) |>
  group_by(strain_type, mybins) |>
  summarise(mean_val = mean(cluster_area),
...
p1 <- ggplot(data, aes(volume, cluster_area, color=strain_type)) +
  geom_point(size=2.0, alpha=0.4) +
  geom_errorbar(data=data_bins, mapping=aes(mean_x, mean_val, ymax=mean_val + sd_val, ymin=mean_val-sd_val),
                linewidth=1.8, width=5) +
  geom_line(data=data_bins, mapping=aes(mean_x, mean_val), linewidth=1.5) +
  labs(y = "Actin cluster area [µm²]",
       x = "Cell volume [fL]",
       title = "Title in progress") +
  scale_color_manual(values = my_col, name = "Hormone conc.") +
  scale_fill_manual(values = my_col, name = "Hormone conc.") +
...
p2 <- ggplot(data, aes(strain_type, cluster_area, fill=strain_type)) +
  geom_boxplot(width = 0.25) +
  labs(y = "Actin cluster area [µm²]",
       x = "",
       title = "Title in progress") +
  scale_fill_manual(values = my_col, name = "Hormone conc.") +
  scale_x_discrete(limits = c("WT", "bni1Δ")) +
  theme_bw(base_size = 14) +
  theme(legend.position = "none",
        axis.title = element_text(color="grey10"),
        plot.title = element_text(color="grey10", face ="bold"),
...
ggsave(file.path(dir_save, "Acting_phalloidin_vol.svg"), p1, width = BASE_WIDTH, height = BASE_HEIGHT)
ggsave(file.path(dir_save, "Acting_phalloidin_box.svg"), p2, width = BASE_WIDTH, height = BASE_HEIGHT)
