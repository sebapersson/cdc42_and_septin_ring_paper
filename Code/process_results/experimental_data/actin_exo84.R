...
data_f <- read_csv(file_path, col_types = cols()) |> 
  mutate(log10_volume = log10(cell_vol_fl_mean_3_frames_emerg), 
         log10_ring_diameter = log10(max_cdc10_ring_diameter))
mybins <- cut(data_f$log10_volume, breaks=8)
data_foo <- data_f |> 
  mutate(mybins = mybins) |> 
  group_by(strain_type, mybins) |> 
  summarise(mean_val = mean(log10_ring_diameter), 
            sd_val = sd(log10_ring_diameter) / sqrt(n()), 
            n = n()) |> 
  filter(n > 5)
valx <- data_f |> 
  mutate(mybins = mybins) |> 
  group_by(mybins) |> 
  summarise(mean_x = mean(log10_volume))
data_bins <- inner_join(data_foo, valx)

my_col <- cbPalette[c(4, 2)]
p1 <- ggplot(data_f, aes(log10_volume, log10_ring_diameter, color=strain_type)) + 
  ...
  labs(y = "log10(Max Cdc11 diameter) [µm]", 
       x = "log10(Mean cell volume three frames after bud emergence) [fL]", 
...
data_e <- read_csv(path_data, col_types = cols()) |> 
  mutate(logvolume = log10(cell_vol_at_max_exo84_diam_fl), 
         logdiameter = log10(max_exo84_cluster_diameter))
mybins <- cut(data_e$logvolume, breaks=8)
data_foo <- data_e |> 
  mutate(mybins = mybins) |> 
  group_by(strain_type, mybins) |> 
...
data_bins <- inner_join(data_foo, valx)
p2 <- ggplot(data_e, aes(logvolume, logdiameter, color=strain_type)) + 
  geom_point(size=2.0, alpha=0.2) + 
  geom_errorbar(data=data_bins, mapping=aes(mean_x, mean_val, ymax=mean_val + sd_val, ymin=mean_val-sd_val), 
                linewidth=1.8, width=0.05) + 
  geom_line(data=data_bins, mapping=aes(mean_x, mean_val), linewidth=1.5) + 
...
p3 <- ggplot(data_g, aes(max_cdc10_diameter, max_exo84_diameter, color=strain_type)) +
  geom_point(size=2.0, alpha=0.2) + 
  geom_errorbar(data=data_bins, mapping=aes(mean_x, mean_val, ymax=mean_val + sd_val, ymin=mean_val-sd_val), 
                linewidth=1.8, width=1) + 
  geom_line(data=data_bins, mapping=aes(mean_x, mean_val), linewidth=1.5) + 
  labs(y = "Max Exo84 diameter [µm]", 
...
ggsave(file.path(dir_save, "Exo84_vol.svg"), p1, width = BASE_WIDTH, height = BASE_HEIGHT)
ggsave(file.path(dir_save, "Cdc11_vol.svg"), p2, width = BASE_WIDTH, height = BASE_HEIGHT)
...
data_b <- tibble(cluster_area = abp140_cluster_area, 
  ...
  bind_rows(tibble(cluster_area = bni1del_cluster_area, 
                   ...
                   strain_type = "Mutant")) |> 
  mutate(logvolume = log10(cell_volume), 
         logarea = log10(cluster_area))
mybins <- cut(data_b$logvolume, breaks=8)
data_foo <- data_b |> 
  mutate(mybins = mybins) |> 
  group_by(strain_type, mybins) |> 
  summarise(mean_val = mean(log10(cluster_area+1e-9)), 
            sd_val = sd(log10(cluster_area+1e-9)) / sqrt(n()), 
            n = n()) |> 
  filter(n > 10) |> 
  filter(strain_type != "Mutant") |> 
  drop_na()
valx <- data_b |> 
  mutate(mybins = mybins) |> 
  group_by(mybins) |> 
...
p1 <- ggplot(data_b, aes(logvolume, logarea, color=strain_type)) +
